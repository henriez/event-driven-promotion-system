...
echo "Starting MS Authentication..."
(cd ms-authentication && ./mvnw clean -e spring-boot:run -Dspring-boot.run.jvmArguments="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5002" > ../logs/authentication.log 2>&1) &
